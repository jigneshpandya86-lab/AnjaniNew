...
export function renderRecentPayments() {
  ...
  list.innerHTML = recent.map((p, index) => {
    // Parse date parts directly without creating a Date object
...
export function submitPayment(e) {
  ...
  const c = DB.customers.find(x => String(x.id) === String(custId));
...
export function shareStatement(id) {
  ...
  if (!txns.length) msg += "(No transactions in this period)\n";
    ...
    if(t.type==='ORD'){
      runningBal+=+t.amt;
      msg+=`🔸 *${fmt(t.date)}* | Delivery\n${t.desc?'📍 '+t.desc.replace(/[()]/g,'')+'\n':''}📦 ${t.qty} Boxes | ₹${t.amt}\n\n`;
    }else{
      runningBal-=+t.amt;
      msg+=`💚 *${fmt(t.date)}* | Payment\n💵 Cash | -₹${t.amt}\n\n`;
    } 
  });
